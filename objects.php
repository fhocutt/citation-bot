...
function capitalize_title($in, $sents = TRUE, $could_be_italics = TRUE) {
  ...
  // <em> tags often go missing around species namesin CrossRef
...
class Page {
  ...
  public function get_text_from($title) {
    ...
    if (strpos($title, "Template:Cite") !== FALSE) {
      $this->cite_template = TRUE;
    }

    if ($this->cite_template && !$this->text) {
      $this->text = $cite_doi_start_code;
    }
    ...
    if ($this->has_reflist) {
      ...
      foreach ($duplicate_refs as $dup) {
        ...
        foreach ($dup as $instance) {
          ...
          if (!$dup_name) {
            $dup_name = get_name_for_reference($long_refs[$name_giver]);
          }

          if ($instance != $name_giver) {
            $long_refs[$instance]->shorten($dup_name);
          }
    ...
    if ($pmid_to_do) {
      foreach ($pmid_to_do as $pmid) {
        ...
        case 1: // Page exists; we need to check that the redirect has been created.
          ...
          break;
        case 2:echo "Database lists page as a redirect";
          break;
        }
      }
    }
  ...
  public function edit_summary() {
    $auto_summary = "";
    if ($this->modifications["changeonly"]) {
    ...
    if ($this->modifications['ref_names']) {
      $auto_summary .= 'Named references. ';
    }
  ...
  public function generate_template_name($replacement_name) {
    ...
    if (preg_match("~^[\d\s]*$~", $replacement_name)) {
      $replacement_name = "ref" . $replacement_name;
    }

    if (!$this->ref_names || !in_array($replacement_name, $this->ref_names)) {
