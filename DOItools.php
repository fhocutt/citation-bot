...
function pmFullTextUrl($pmid) {
  ...
  if ($xml) {
    foreach ($xml->LinkSet->IdUrlList->IdUrlSet->ObjUrl as $url) {
      foreach ($url->Attribute as $attrib) {
        $requiredFound = strpos($url->Attribute, "required") ? true : $requiredFound;
      }
...
function get_data_from_isbn() {
  ...
  foreach ($params as $null) {
    $missingInfo = true;
  }

  if ($missingInfo) {
    $xml = simplexml_load_file("http://xisbn.worldcat.org/webservices/xid/isbn/" . str_replace(array("-", " "), "", $p["isbn"][0]) . "?method=getMetadata&fl=*&format=xml");
  }
#&ai=Wikipedia_doibot");
  if ($xml["stat"] == "ok") {
    ...
    if (substr($p["author"][0], 0, 3) == "by ") {
      $p["author"][0] = substr($p["author"][0], 3);
    }

    if (preg_match("~\d+~", $p["oclc"][0], $match)) {
      $p["oclc"][0] = $match[0];
    }
    ...
    if ($xml->BookList["total_results"] > 0) {
      ...
      if (is("author") || is("first") || is("author1") || is("author1")) {
        unset($params["author"]);
      }

      foreach ($params as $key => $value) {
        if (!is($key)) {
          $p[$key][0] = capitalize_title((string) $xml->BookList->BookData->$value);
        }
      }
      ...
      $p["ISBN-status"][0] = "May be invalid - please double check";
    }
...
//Copied from WWW.
  ...
  if (!$fp) {
    ...
    $headers = "";
    $startTime = time();
    while (!@feof($fp) && strlen($headers) < 2500 && $startTime + 2 > time()) {
  ...
  if (substr($url, -3) == "pdf") {
    ...
    if (!@array_search($url, $urlsTried)
      ...
      if (preg_match("|[=/](10\.\d{4}/[^?&]*)|", str_replace("%2F", "/", $url), $doiX)) {
        ...
        if (preg_match("~spanclasstitle" . $lTitle . "~", $lSource)) {echo "DOI found in URL. Web page has correct title.<br>";return $doiX[1];} else {
      ...
      if (!$doi) {$urlsTried[] = $url;
        echo "URL is barren.<br>";return false;}
  ...
  if (preg_match("~" . $lTitle . "~", $lSource, $titleOffset, PREG_OFFSET_CAPTURE)) {
    ...
    if (preg_match_all("~doi.*" . doiRegexp . "~Ui", $source, $dois)) { // What comes after "doi", then any nonword, but before whitespace
      if ($dois[1][2]) {
        ...
        if ($testDoi) {$doi = testDoi(trimEnd($dois[1][0]));if ($doi) {
          return $doi;
        }
        } // DOI redirects to our URL so MUST be correct
    ...
    } else {echo "\nNo DOIs found in page.";
      $urlsTried[] = $url;RETURN false;}
  }
...
function formatSurname($surname) {
  ...
  } else if (mb_substr($surname, 0, 2) == "mc") {
    return "Mc" . fmtSurname2(mb_substr($surname, 2));
  } else if (mb_substr($surname, 0, 3) == "mac" && strlen($surname) > 5 && !mb_strpos($surname, "-") && mb_substr($surname, 3, 1) != "h") {
    return "Mac" . fmtSurname2(mb_substr($surname, 3));
  } else if (mb_substr($surname, 0, 1) == "&") {
    return "&" . fmtSurname2(mb_substr($surname, 1));
  } else {
    return fmtSurname2($surname);
  }
  // Case of surname
}
...
function isInitials($str) {
  ...
  if (strlen(str_replace(array("-", ".", ";"), "", $str)) > 3) {
    return false;
  }

  if (strlen(str_replace(array("-", ".", ";"), "", $str)) == 1) {
    return true;
  }
  ...
  //Otherwise we've got no handy comma to separate; we'll have to use spaces and periods.
    ...
    if (isset($auth[1])) {
      ...
      foreach ($chunks as $chunk) {
        if (!$surname && !isInitials($chunk)) {
          $surname = $chunk;
        } else {
          array_unshift($i, isInitials($chunk) ? formatInitials($chunk) : $chunk);
        }
...
function formatAuthors($authors, $returnAsArray = false) {
  ...
  if (preg_match("~[,;]$~", trim($authors))) {
    $authors = substr(trim($authors), 0, strlen(trim($authors)) - 1);
  }
...
// could be first author, or an author with no initials, or a surname with initials to follow.
    }
  }
  if ($savedChunk) {
    $return[0] = $bits[0];
...
function get_all_meta_tags($url) {
  if (preg_match("~http://www\.pubmedcentral\.nih\.gov/articlerender\.fcgi\?artinstid=(\d*)~", $url, $pmc)) {
    $return["pmc"] = $pmc[1];
  }
  ...
  if ($creators[1][1]) {
    foreach ($creators[1] as $aut) {
      if (mb_strtoupper($aut) == $aut) {
        ...
        $count = count($aut);
        for ($i = 0; $i <= $count; $i += 2) {
          $return["author"] .= $aut[$i] . (isset($aut[$i + 1]) ? " " : "") . $aut[$i + 1];
        }
...
function testDoi($doi) {
  ...
  if ($p["url"][0]) {
    if (strpos($p["url"][0], "http://dx.doi.org/") === 0) {
      return $doi;
    }
    ...
    if (equivUrl($effectiveUrl) == equivUrl($p["url"][0])) {
      return $doi;
    } else {
      return false;
    }
